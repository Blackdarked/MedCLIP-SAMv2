...
\textbf{Chuẩn hóa (Normalization):} Ảnh được chuẩn hóa về thang giá trị phù hợp với backbone sử dụng. Với các mô hình kiểu CLIP/BiomedCLIP, có thể biểu diễn bằng công thức:
\begin{equation}
I_{\mathrm{norm}} = \frac{I - \mu}{\sigma}
\end{equation}
Trong đó, $\mu$ và $\sigma$ là giá trị trung bình và độ lệch chuẩn của tập tiền huấn luyện.
...
\centering
\includegraphics[width=\linewidth,keepaspectratio]{breast_sample.png}
\caption{\normalfont Breast US (BUSI / UDIAT)}
\label{fig:sample_breast}
\end{subfigure}

\vspace{0.35em}
\begin{subfigure}{\linewidth}
\centering
\includegraphics[width=\linewidth,keepaspectratio]{brain_sample_0001.png}
\caption{\normalfont Brain MRI (Br35H)}
\label{fig:sample_brain}
\end{subfigure}

\vspace{0.35em}
\begin{subfigure}{\linewidth}
\centering
\includegraphics[width=\linewidth,keepaspectratio]{xray_sample_0001.png}
\caption{\normalfont Lung X-ray (Montgomery + Shenzhen)}
\label{fig:sample_xray}
\end{subfigure}

\vspace{0.35em}
\begin{subfigure}{\linewidth}
\centering
\includegraphics[width=\linewidth,keepaspectratio]{ct_sample.png}
\caption{\normalfont Lung CT (LUNA16)}
\label{fig:sample_ct}
\end{subfigure}
\caption{\normalfont Ví dụ ảnh mẫu từ bốn tập dữ liệu thực nghiệm (ảnh đầu vào, ground truth, kết quả zero-shot và nnU-Net).}
